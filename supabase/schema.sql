...
-- v4.35: salas de chat, suporte privado e topicos da comunidade.
...
create table if not exists public.chat_sala_membros (
  ...
  usuario_id uuid not null references public.usuarios(id) on delete cascade,
  papel text not null default 'membro', adicionado_em timestamptz not null default now(),
...
create table if not exists public.suporte_conversas (
  id uuid primary key default gen_random_uuid(), usuario_id uuid not null unique references public.usuarios(id) on delete cascade,
  status text not null default 'aberta' check(status in ('aberta','fechada')),
  criado_em timestamptz not null default now(), atualizado_em timestamptz not null default now()
);
create table if not exists public.suporte_mensagens (
  id bigint generated by default as identity primary key, conversa_id uuid not null references public.suporte_conversas(id) on delete cascade,
  autor_id uuid not null references public.usuarios(id) on delete cascade, mensagem text not null,
  criado_em timestamptz not null default now()
);
create table if not exists public.topicos_comunidade (
  id bigint generated by default as identity primary key, autor_id uuid not null references public.usuarios(id) on delete cascade,
  titulo text not null, conteudo text not null, categoria text not null default 'duvida', fechado boolean not null default false,
  criado_em timestamptz not null default now(), atualizado_em timestamptz not null default now()
);
create table if not exists public.topico_respostas (
  id bigint generated by default as identity primary key, topico_id bigint not null references public.topicos_comunidade(id) on delete cascade,
...
create table if not exists public.xp_eventos (
  id bigint generated by default as identity primary key,
  usuario_id uuid not null references public.usuarios(id) on delete cascade,
  chave text not null, tipo text not null, pontos integer not null check (pontos > 0),
  detalhes jsonb not null default '{}'::jsonb, criado_em timestamptz not null default now(),
...
create table if not exists public.notificacoes (
  ...
  titulo text not null, mensagem text not null, acao text, referencia_id text, chave text not null,
  lida_em timestamptz, criado_em timestamptz not null default now(), unique(usuario_id,chave)
);
...
create or replace function public.conceder_xp(p_usuario_id uuid,p_chave text,p_tipo text,p_pontos integer,p_detalhes jsonb default '{}'::jsonb)
returns table(aplicado boolean,xp_total bigint)
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_id bigint; v_total bigint;
begin
  if p_usuario_id is null or nullif(trim(p_chave),'') is null or p_pontos <= 0 then raise exception 'Evento de XP inválido.'; end if;
  ...
  on conflict(usuario_id,chave) do nothing returning id into v_id;
  if v_id is not null then update public.usuarios as u set xp_total=u.xp_total+p_pontos where u.id=p_usuario_id returning u.xp_total into v_total;
  else select u.xp_total into v_total from public.usuarios u where u.id=p_usuario_id; end if;
...
declare
  v_total bigint; v_usado bigint:=0; v_concedido integer:=0;
  v_evento_id bigint; v_inicio timestamptz;
begin
  if p_usuario_id is null or nullif(trim(p_chave),'') is null or p_pontos<=0 or p_limite_diario<=0 then
  ...
  select u.xp_total into v_total from public.usuarios u where u.id=p_usuario_id for update;
  if not found then raise exception 'Usuário não encontrado.'; end if;
  if exists(select 1 from public.xp_eventos e where e.usuario_id=p_usuario_id and e.chave=trim(p_chave)) then
    return query select false,v_total,0; return;
  end if;
  v_inicio:=date_trunc('day',now() at time zone 'America/Belem') at time zone 'America/Belem';
  select coalesce(sum(e.pontos),0) into v_usado from public.xp_eventos e
  ...
  if v_concedido<=0 then return query select false,v_total,0; return; end if;
  insert into public.xp_eventos(usuario_id,chave,tipo,pontos,detalhes)
  values(p_usuario_id,trim(p_chave),p_tipo,v_concedido,
    coalesce(p_detalhes,'{}'::jsonb)||jsonb_build_object('limite_diario',p_limite_diario))
  on conflict(usuario_id,chave) do nothing returning id into v_evento_id;
  if v_evento_id is null then return query select false,v_total,0; return; end if;
  update public.usuarios u set xp_total=u.xp_total+v_concedido where u.id=p_usuario_id
  returning u.xp_total into v_total;
  return query select true,v_total,v_concedido;
end; $$;
revoke all on function public.conceder_xp_questao_limitado(uuid,text,text,integer,jsonb,integer) from public,anon,authenticated;
grant execute on function public.conceder_xp_questao_limitado(uuid,text,text,integer,jsonb,integer) to service_role;
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_bonus bigint; v_removidos bigint;
begin
  select coalesce(u.xp_bonus_plano,0)::bigint into v_bonus
  from public.usuarios as u where u.id=p_usuario_id for update;
  if not found then raise exception 'Usuário não encontrado.'; end if;
  delete from public.xp_eventos as e where e.usuario_id=p_usuario_id and e.tipo<>'plano';
  get diagnostics v_removidos=row_count;
  delete from public.notificacoes as n where n.usuario_id=p_usuario_id and n.tipo in ('missao','patente');
  update public.usuarios as u set xp_total=v_bonus,patente_notificada_nivel=0,papirao_notificado=false
...
language plpgsql security definer set search_path=public,pg_temp as $$
declare v_alvo integer; v_diferenca integer;
begin
  v_alvo:=case when new.perfil in ('admin','supremo') then 0 when new.vip or new.plano_atual='vitalicio' then 2500 when new.plano_atual='trimestral' then 1200 when new.premium or new.plano_atual='mensal' then 500 else 0 end;
  v_diferenca:=greatest(v_alvo-coalesce(new.xp_bonus_plano,0),0);
  if v_diferenca>0 then
    perform public.conceder_xp(new.id,'bonus-plano-'||v_alvo,'plano',v_diferenca,jsonb_build_object('bonus_total',v_alvo,'plano',new.plano_atual));
    ...
    values(new.id,'plano','Bônus de assinatura','Seu plano concedeu '||v_diferenca||' XP de bônus permanente.','missoes','plano-xp-'||v_alvo)
