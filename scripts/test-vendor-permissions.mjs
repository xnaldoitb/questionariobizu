...
const [usersRoute, paymentsRoute, usersUi, permissions] = await Promise.all([
    ...
    readFile(new URL('../server/platform/admin-permissions.mjs', import.meta.url), 'utf8'),
]);
...
assert.ok(usersRoute.includes('claimUnassignedAdminTarget(actor, target)'));
...
assert.ok(paymentsRoute.includes('claimUnassignedAdminTarget(actor, target)'));
assert.ok(permissions.includes('target.responsavel_admin_id === actor?.id'));
