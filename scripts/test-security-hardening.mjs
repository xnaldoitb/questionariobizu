...
const adminPermissionsSource = await readFile(new URL('../server/platform/admin-permissions.mjs',import.meta.url),'utf8');
...
assert(userAdminSource.includes('canManageTarget(actor, target)'));
assert(paymentAdminSource.includes('canManage(actor, target)'));
assert(adminPermissionsSource.includes('target.responsavel_admin_id === actor?.id'));
