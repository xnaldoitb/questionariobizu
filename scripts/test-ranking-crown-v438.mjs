...
assert(performance.includes("patentButtonMarkup(entry.acertos, { papirao: true })"));
...
assert(performance.includes("<span class=\"podium-position\">${rankBadge(index)} ${index + 1}º</span>${patentButtonMarkup(entry.acertos, { developer: developerEntry(entry) })}"));
assert(!performance.includes('ranking-avatar'));
assert(performance.includes('${safeText(entry.nome)} ${accountBadges(entry)}'));
