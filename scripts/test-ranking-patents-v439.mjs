...
assert.equal(patentForHits(50000).name, 'Segundo-Tenente da Estratégia I');
assert.equal(patentForHits(43000).name, 'Subtenente do Conhecimento');
assert.equal(patentForHits(66500).name, 'Oficial de Questões I');
assert.equal(patentForHits(303000).name, 'Comandante do Saber');
assert.equal(patentForHits(999999).name, 'Herói do Conhecimento');
assert.equal(patentProgress(199).remaining, 1);
assert.equal(patentStatus(320000).progresso, 100);
...
assert(patentInsigniaMarkup(200).includes('patent-enlisted-bars'));
assert(patentInsigniaMarkup(1750).includes('patent-sergeant-chevrons'));
assert(patentInsigniaMarkup(1750).includes('patent-sergeant-base'));
assert(patentInsigniaMarkup(2750).includes('patent-sergeant-tier-1'));
assert(patentInsigniaMarkup(7500).includes('patent-sergeant-tier-2'));
assert.equal((patentInsigniaMarkup(2750).match(/class="patent-chevron"/g) || []).length, 2);
assert.equal((patentInsigniaMarkup(4000).match(/patent-progressive-base/g) || []).length, 2);
assert.equal((patentInsigniaMarkup(5500).match(/patent-progressive-base/g) || []).length, 3);
assert.equal((patentInsigniaMarkup(7500).match(/class="patent-chevron"/g) || []).length, 3);
assert(patentInsigniaMarkup(7500).includes('patent-progressive-dots'));
assert.equal((patentInsigniaMarkup(7500).match(/class="patent-grade-dot"/g) || []).length, 1);
assert.equal((patentInsigniaMarkup(10000).match(/class="patent-grade-dot"/g) || []).length, 2);
assert.equal((patentInsigniaMarkup(13000).match(/class="patent-grade-dot"/g) || []).length, 3);
assert(patentInsigniaMarkup(20500).includes('patent-senior-sergeant'));
assert.equal((patentInsigniaMarkup(20500).match(/class="patent-chevron"/g) || []).length, 4);
assert.equal((patentInsigniaMarkup(20500).match(/class="patent-star"/g) || []).length, 1);
assert(patentInsigniaMarkup(43000).includes('patent-subtenente-triangle'));
assert.equal((patentInsigniaMarkup(43000).match(/class="patent-chevron"/g) || []).length, 0);
assert.equal((patentInsigniaMarkup(43000).match(/class="patent-star"/g) || []).length, 1);
assert.equal(new Set(PATENTS.slice(4, 17).map((patent) => patentInsigniaMarkup(patent.min))).size, 13);
assert(patentInsigniaMarkup(50000).includes('patent-officer-pips'));
assert(patentInsigniaMarkup(92000).includes('patent-officer-stars'));
assert(patentInsigniaMarkup(124000).includes('patent-field-officer'));
...
assert.equal((patentInsigniaMarkup(124000).match(/patent-major-grade/g) || []).length, 1);
assert.equal((patentInsigniaMarkup(154000).match(/patent-major-grade/g) || []).length, 5);
assert.equal((patentInsigniaMarkup(162000).match(/patent-lieutenant-grade/g) || []).length, 1);
assert.equal((patentInsigniaMarkup(197000).match(/patent-lieutenant-grade/g) || []).length, 5);
assert(patentInsigniaMarkup(303000).includes('patent-command-50'));
assert(patentInsigniaMarkup(303000).includes('patent-command-stars'));
assert(patentInsigniaMarkup(320000).includes('patent-hero-marks'));
assert(patentInsigniaMarkup(320000).includes('patent-hero-cluster'));
assert(!patentInsigniaMarkup(303000).includes('patent-command-wing'));
assert(!patentInsigniaMarkup(320000).includes('patent-hero-wings'));
assert.equal((patentInsigniaMarkup(303000).match(/class="patent-star"/g) || []).length, 5);
assert.equal((patentInsigniaMarkup(320000).match(/class="patent-star"/g) || []).length, 6);
assert(patentInsigniaMarkup(320000).includes('patent-tile-frame'));
...
console.log('Ranking v4.44: nomes completos, patentes por XP e progressão institucional validados.');
