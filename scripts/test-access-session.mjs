...
assert(node('#paymentModal').classList.contains('hidden'));
assert(node('#automaticPaymentNotice').classList.contains('hidden'));
assert.equal(node('#automaticPaymentNotice').textContent, '');
...
assert(node('#automaticPaymentNotice').classList.contains('hidden'));
assert([...timers.values()].some(t=>t.ms===30000));
console.log('Sessão: planos na entrada sem compra involuntária, Premium automático e pausa/retomada passaram (DOM/API simulados).');
console.log('Cobrança: oculta para Premium/VIP/ADM/Desenvolvedor e disponível somente para aluno sem plano ativo.');
