...
export const handler = async (event) => {
    ...
    try {
        ...
        if (action === 'end_student_sessions') {
            const { error: deviceError, count: deviceCount } = await db()
            ...
            const { error, count } = await db()
                .from('usuarios')
                .update(
                    { sessao_ativa_id: null, sessao_ativa_expira_em: null, sessao_ativa_device_hash: null },
                    { count: 'exact' },
                )
                .eq('perfil', 'aluno')
            ...
            if (error) throw error;
            return json(200, { ok: true, sessoes_encerradas: deviceCount || count || 0 });
        }

        return json(400, { erro: 'Ação de manutenção inválida.' });
    } catch (error) {
        console.error('Falha na manutenção administrativa:', error.message);
        return json(400, { erro: 'Não foi possível concluir a manutenção.' });
    }
};
