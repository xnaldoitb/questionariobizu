...
async function loadFromView() {
    const { data, error } = await db()
        ...
        .select('usuario_id,nome,usuario,perfil,vip,sessoes,respondidas,acertos,percentual');
...
async function loadFallback() {
    ...
    while (true) {
        const { data, error } = await db()
            .from('respostas')
            .select('id,usuario_id,sessao_id,acertou,usuarios(nome,usuario,perfil,vip)')
            .eq('pulada', false)
