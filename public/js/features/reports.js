...
async function loadHistoryData({ append = false } = {}) {
    ...
    try {
        ...
        if (!historyResponses.length && Number(data.sessoes_total || 0) > 0) {
            $('#historyList').innerHTML = '<div class="report-empty">Nenhuma resposta detalhada encontrada para este filtro.</div>';
            return;
        }
...
function renderHistoryResponses() {
    ...
    const loadMore = historyHasMore
        ? `<div class="history-load-more-wrap">
            <button class="btn ghost history-load-more" id="historyLoadMore" type="button">Carregar mais 20</button>
           </div>`
        : '';

    $('#historyList').innerHTML = historyResponses.length
        ? cards + loadMore
    ...
    $('#historyLoadMore')?.addEventListener('click', async () => {
        historyPage += 1;
        const button = $('#historyLoadMore');
        if (button) {
            button.disabled = true;
            button.textContent = 'Carregando…';
        }
        await loadHistoryData({ append: true });
    });
}
