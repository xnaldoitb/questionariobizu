...
async function loadAllQuestions(disciplineId, chapterId) {
    const questions = [];
    let page = 1;
...
async function startQuiz() {
    try {
        ...
        try {
            if (allQuestions) {
                state.quiz.questions = await loadAllQuestions(disciplineId, chapterId);
            } else {
                const data = await api(
                    `questoes?disciplina=${encodeURIComponent(disciplineId)}` +
                    `&capitulo=${encodeURIComponent(chapterId)}` +
                ...
                state.quiz.questions = data.questoes.slice(0, Number(limit));
            }
        } finally {
